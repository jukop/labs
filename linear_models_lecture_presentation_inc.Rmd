...
\[
\texttt{friction} = \beta_0 + \beta_1\mathbf{1}(\texttt{type=push}) + \beta_2\mathbf{1}(\texttt{leg=L2}) + \beta_3 \mathbf{1}(\texttt{leg=L3}) + \beta_4 \mathbf{1}(\texttt{leg=L4}) + \varepsilon_i,
\]
where $i=1,\dots,n$
...
There will be an update on exercises:
 ...
 - Topic of confounding is noted not needed to be completed.