...
We will describe two examples from the life sciences: one related to genetics and one from a mouse experiment. They are very different, yet we end up using the same statistical technique: fitting linear models. Linear models are typically taught and described in the language of matrix algebra. 
...
```{r galton_data, fig.cap="Galton's data. Son heights versus father heights.", out.width="50%"}
...
```{r mice_weights,fig.cap="Mouse weights under two diets.", out.width="50%"}
...
We have seen two very different examples in which linear models can be used. A general model that encompasses all of the above examples is the following:
...
```{r rss_versus_estimate, fig.cap="Residual sum of squares obtained for several values of the parameters.", out.width="50%"}
Beta0s<- seq(-20,70,len=100)
plot(Beta0s,sapply(Beta0s,rss,Beta1=0.5),
     ylab="RSS",xlab="Beta0",type="l")
##Let's add another curve fixing another pair:
Beta0s<- seq(-20,70,len=100)
lines(Beta0s,sapply(Beta0s,rss,Beta1=0.6),col=2)
```
...
## Advanced: Linear or nonlinear?

Actually the term linear in the "linear models" refers to linearity with respect to betas. The linear model can have nonlinear function of predictors (but not nonlinear function of betas), e.g. $\phi(x) = x^2$, instead of predictors themselves in the equation. In that case the linear model equation becomes
\[
  Y_i = \beta_0 + \beta_1 \phi_1(x_{i,1}) + \beta_2 \phi_2(x_{i,2}) + \dots +  \beta_2 \phi_p(x_{i,p}) + \varepsilon_i, i=1,\dots,n
\]

The matrix algebra is able to use Least Squares to also estimate this kind of models. If you make a model to be a nonlinear function of beta-parameters, e.g. 
\[Y_i = \beta_0(1-e^{-\beta_1 X_i}) + \varepsilon_i\]
so that is **not** a linear model!

Actually that model is something called Von Bertalanffy growth model (<https://en.wikipedia.org/wiki/Von_Bertalanffy_function>) which is often used in modelling of fish growth. 

That is a special case of Richard's curve, which has been used in modelling the spread of COVID-19 pandemic (<https://en.wikipedia.org/wiki/Generalised_logistic_function#Modeling_COVID-19_infection_trajectory>). If you want to understand more of these kinds of special models, then you need to understand linear models first!
...
```{r gravity, fig.cap="Simulated data for distance travelled versus time of falling object measured with error.", out.width="50%"}
mypar()
...
```{r simulate_drop_data_with_fit, fig.cap="Fitted model for simulated data for distance travelled versus time of falling object measured with error.", out.width="50%"}
...
How do we find the LSE? Well, it works the same way it was presented earlier at these slides.
...
![Galton's plot.](./figure/Galton's_correlation_diagram_1875.jpg)
